...
# Compatibility shim for GluonTS 0.14.4+ (loss module was removed)
try:
    from gluonts.torch.modules.loss import DistributionLoss, NegativeLogLikelihood
except ImportError:
    class DistributionLoss:
        """Fallback for GluonTS 0.14.4+ where this module was removed."""
        def __call__(self, distribution, target):
            return -distribution.log_prob(target)
    class NegativeLogLikelihood(DistributionLoss):
        """Fallback NegativeLogLikelihood loss."""
        pass
...
class LagLlamaEstimator(PyTorchLightningEstimator):
    ...
    def create_lightning_module(self, use_kv_cache: bool = False) -> pl.LightningModule:
        ...
        if self.ckpt_path is not None:
            # PyTorch 2.6+ requires safe globals for checkpoints with custom classes
            import torch
            from gluonts.torch.distributions import StudentTOutput, NegativeBinomialOutput
            safe_classes = [
                self.distr_output.__class__,
                StudentTOutput,
                NegativeBinomialOutput,
                DistributionLoss,
                NegativeLogLikelihood,
            ]
            # Filter out any None or duplicate classes
            safe_classes = list(set([c for c in safe_classes if c is not None]))
